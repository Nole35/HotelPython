...
def dodajHotel():
    hoteli = fajlovi.citanjeHotela()
    
    id_hotela = fajlovi.generisanjeId(hoteli, "id_hotela")
    naziv = input("Unesite naziv hotela: ")
    adresa = input("Unesite adresu hotela: ")
    lista_soba = input("Unesite brojeve soba: ")
    ...
    hoteli.append(hotel)
    fajlovi.snimanjeHotela(hoteli)


def pregledRecepcionera():
    lista_recepcionera = fajlovi.citanjeKorisnika()
...
def pregledHotela():
    lista_hotela_brisanje = fajlovi.citanjeHotela()
...
def pretragaRecepcionera():
    while True:
        ...
        if opcija == "1":
            ...
            print("!GRESKA! Niste unijeli odgovarajucu funkciju!")
...
def pretragaJedanKriterjium():
    while True:
        ...
        opcija = input("Unesi zeljenu opciju: ")
        lista_korisnika = fajlovi.citanjeKorisnika()
        lista_recepcionera = []
        ...
        if opcija == "1":
            ...
            kriterijum = input("Unesite rijec za pretragu: ")
            recepcioneriPretraga = []
            for recepcioner in lista_recepcionera:
...
def pretragaViseKriterjiuma():
    ...
    for k in lista_korisnika:
        if k["vrsta_korisnika"] == "recepcioner" and k["aktivan"] == "True":
            lista_recepcionera.append(k)
    kriterijum = input("Unesite rijeci za pretragu odvojene zarezom: ")
    recepcioneriPretraga = []
...
def brisanjeHotela():
    lista_hotela = fajlovi.citanjeHotela()
    ...
    while True:
        ...
        if dobarUnos:
            ...
            print("Ne postoji hotel sa unijetim ID-em! ")
    for h in lista_hotela:
        if h["id_hotela"] == idHotel:
            h["aktivan"] = "False"
    fajlovi.snimanjeHotela(lista_hotela)
    ...
    for k in lista_korisnika:
        if k["vrsta_korisnika"] == "recepcioner":
            if k["hotelID"] == idHotel:
                k["aktivan"] = "False"
    fajlovi.snimanjeKorisnika(lista_korisnika)

    lista_sobe = fajlovi.citanjeSoba()
    for soba in lista_sobe:
        if soba["broj_sobe"] in sobeHotela:
            soba["aktivan"] = "False"
    fajlovi.snimanjeSoba(lista_sobe)

    lista_rezervacija = fajlovi.citanjeRezervacije()
    for rez in lista_rezervacija:
        for brojSobe in sobeHotela:
            if brojSobe in rez["rez_sobe"]:
                rez["aktivan"] = "False"
    fajlovi.snimanjeRezervacije(lista_rezervacija)
...
def brisanjeRecepcionera():
    ...
    while True:
        korIme = input("Unesite korisnicko ime recepcionera za brisanje: ")
        dobarUnos = False
...
def dodajSobu():
    ...
    while True:
        ...
        if dobarUnos:
            ...
            print("Ne postoji hotel sa unijetim ID-em! ")
    lista_soba = fajlovi.citanjeSoba()
    id_sobe = fajlovi.generisanjeId(lista_soba, "id_sobe")
...
def dodajBazen():
    ...
    while True:
        idHotel = input("Unesite ID hotela gdje zelite promijeniti stanje za bazen: ")
        dobarUnos = False
        for b in azuriranje_hotela:
            if b["id_hotela"] == idHotel:
                dobarUnos = True
...
def dodajRestoran():
    ...
    while True:
        idHotel = input("Unesite ID hotela gdje zelite promijeniti stanje za restoran: ")
        dobarUnos = False
        for b in azuriranje_hotela:
            if b["id_hotela"] == idHotel:
                dobarUnos = True
